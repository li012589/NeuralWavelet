...
for i in range(4 * repeat):
    layerList.append(torch.nn.Sequential(torch.nn.Conv2d(9, hchnl, 3, padding=1), torch.nn.Hardtanh(inplace=True), torch.nn.Conv2d(hchnl, hchnl, 1, padding=0), torch.nn.Hardtanh(inplace=True), torch.nn.Conv2d(hchnl, 3, 3, padding=1)))
