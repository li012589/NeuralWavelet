...
if args.folder is None:
    rootFolder = './opt/default_easyMera_' + args.target + "_repeat_" + str(args.repeat) + "_nhidden_" + str(args.nhidden) + "_hdim_" + str(args.hdim) + "_hchnl_" + str(args.hchnl) + "_nMixing_" + str(args.nMixing) + "/"
...
if not args.load:
    ...
    with open(rootFolder + "/parameter.json", "w") as f:
        config = {'target': target, 'repeat': repeat, 'nhidden': nhidden, 'hdim': hdim, 'hchnl': hchnl, 'nMixing': nMixing, 'epoch': epoch, 'batch': batch, 'savePeriod': savePeriod, 'lr': lr}
        json.dump(config, f)
else:
