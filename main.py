...
group = parser.add_argument("Target Parameters")
group.add_argument("-target", default="CIFAR")

group = parser.add_argument("Architecture Parameters")
...
if target == "CIFAR":
    ...
    trainsetTransform = torchvision.transforms.Compose([torchvision.transforms.ToTensor()])
...
priorList = []
_length = (blockLength * blockLength) / 4
...
for _ in range(repeat + 1):
    ...
    tList = [utils.SimpleMLPreshape([3 * 2 * 1] + [hdim] * nhidden + [3 * 2 * 1], [nn.ELU(), nn.ELU(), None]) for _ in range(nNice)]
