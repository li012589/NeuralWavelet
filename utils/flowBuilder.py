...
def extractPrior(flowCon):
    layers = []
    _diag = flowCon.layerList[0]
    nn = _diag.shift.shape[0]
    _op = source.Gaussian([nn]).to(torch.float64)
    assert _diag.shift.sum() == 0
    assert _diag.fix.sum() == nn//2
    layers.append(flow.DiagScaling(nn,initValue=_diag.elements.clone().detach()))
    return flow.FlowNet(layers,_op).to(torch.float64)

def extractQPrior(flowCon):
    layers = []
    _diag = flowCon.layerList[0]
    nn = _diag.shift.shape[0]//2
    _op = source.Gaussian([nn]).to(torch.float64)
