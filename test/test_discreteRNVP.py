...
def test_bijective():
    ...
    sList = [utils.SimpleMLPreshape([3 * 32 * 16, 200, 500, 3 * 32 * 16], [nn.ELU(), nn.ELU(), None]) for _ in range(4)]
    f = flow.DiscreteRNVP(maskList, sList, tList, decimal, utils.roundingWidentityGradient, p)

    x, p = f.sample(100)
    z, ip = f.inverse(x)
    xz, gp = f.forward(z)
    op = f.prior.logProbability(z)
    zx, ipp = f.inverse(xz)
