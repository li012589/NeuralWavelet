...
def color_transfer(source, target, clip=True, preserve_paper=True):
    ...
    -------
    source: NumPy array
    CV image in BGR color space (the source image)
    target: NumPy array
    CV image in BGR color space (the target image)
    clip: Should components of L*a*b* image be scaled by np.clip before
    back to BGR color space?
    False then components will be min-max scaled appropriately.
    will keep target image brightness truer to the input.
    will adjust image brightness to avoid washed out portions
    the resulting color transfer that can be caused by clipping.
    preserve_paper: Should color transfer strictly follow methodology
    out in original paper? The method does not always produce
    pleasing results.
    False then L*a*b* components will scaled using the reciprocal of
    scaling factor proposed in the paper.  This method seems to produce
