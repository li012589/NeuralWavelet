...
# Don't have to rebuild the model, skip this part.
'''
...
def divide(z):
    ...
    for no in range(int(math.log(blockLength, 2))):
        ...
        if no != int(math.log(blockLength, 2)) - 1:
            zpart = zpart - decimal.forward_(f.meanList[no]).int() + args.nbins // 2
        else:
            zpart = zpart - (decimal.forward_(f.prior.lastPrior.mean.permute([1, 2, 3, 0])) * torch.softmax(f.prior.lastPrior.mixing, -1)).sum(-1).reshape(1, *f.prior.lastPrior.mean.shape[1:]).int() + args.nbins // 2
